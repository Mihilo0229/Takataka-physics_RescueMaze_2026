...
void setup() {
  ...
  // VL53L0X初期化(番号は左後から反時計回り)
  initializeSensor(sensor1, 28, 0x30);
  initializeSensor(sensor2, 27, 0x31);
  initializeSensor(sensor3, 26, 0x32);
  ...
  Serial.println("Initialization conplete.");
}
...
void loop() {
  ...
  Serial.println(tof_buf[0][tof_idx]);
  Serial.println(tof_buf[1][tof_idx]);
  Serial.println(tof_buf[2][tof_idx]);
  Serial.println(tof_buf[3][tof_idx]);
  Serial.println(tof_buf[4][tof_idx]);
  Serial.println(tof_buf[5][tof_idx]);
